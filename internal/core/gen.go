...
func jdbcGet(t ktType, idx int) string {
	...
	// wasNull() to tell "absent" from "zero". takeUnless keeps the read a single
	// expression, which is what ResultSet() splices in per constructor argument -- and it
	// evaluates the getter before the lambda, so wasNull() still refers to this column.
